...
My understanding of the prefix idea from Umbra is that it is not trying to achieve perfect filtering
on the prefix alone, like some hash function with nearly impossible chances of collisions.
The main point I'm trying to achieve is to get filtering via the short-circuit
check on the prefix, in order reduce the time the query execution needs to spend
in deserializing binary molecules. I don't believe we need to completely avoid deserializing
the binary molecule to achieve good results in queries in duckdb.
We just need to avoid deserializing "enough".

### How many collisions with the current implementation of the "count prefix"

Before starting, I wanted to get a baseline measurement.
The standard implementation of the exact match deserializes on every molecule.
The first implementation of the Umbra-mol utilizes a short-circuit to avoid
deserializing on every molecule in the query. [Part 1] demonstrated a big
speedup when we do not need to deserialize on every molecule.

Here, I wanted to get some more data on how much the initial implementation might
be deserializing (when the short-circuit does not return early), so that I
...
Below, I show the counts of replicated prefixes in descending order.
...
That means for many molecules it is not possible to resolve that they are different
by the prefix alone, and they require the deserialization of the binary molecule to
answer that, and then for some molecules, if you hit one of these hot spots,
...
Indeed, this query runs slower than, for example Query 1 from [part 1]. I logged
each time the short circuit was not triggered and deserialization of the binary molecule
was required, and I got 512 lines (not sure why there is an extra row).
These results seem to support that the more deserialization we need to do in the query execution,
the slower it will be.

Taking a look at the biggest group, they are all distinct SMILES, but look very similar
at a quick glance. They could be stereoisomers, or perhaps even very close analogs
that just happen to hit the same numbers. I don't know, that sounds pretty hard to do.

Anyhow, the point is this gives some indication of how much filtering the prefix
of the counts help in filtering molecules, and a place to work from to see if
a smaller prefix could be at least similar in its filtering capabilities or better.
...
### Trying prefixes from canonical smiles

Since comparing the SMILES between the two molecules is the final check,
why make better use of the bytes in the prefix to store something that might
filter out more?
...
full-separated prefix (20 & 10 byte versions)

### Conclusions so far

- Analysis of the max values of the counts used in the prefix revealed the values
  can likely have no issues being stored in 2 bytes as opposed to 4 bytes, forming a 10 byte
  prefix instead of 20 bytes
- The 10 byte prefix performs similarly to the 20-byte prefix, and perhaps even better (but I didn't do thorough testing
  of the difference to conclude this)
